...
def on_ui_tabs():
    with gr.Blocks() as dreambooth_interface:
                ...
                with gr.Tab("Select"):
                    ...
                    with gr.Row(visible=False) as lora_model_row:
                        db_lora_model_name = gr.Dropdown(
                            label="Lora Model", choices=sorted(get_lora_models())
                        )
                        create_refresh_button(
                            db_lora_model_name,
                            get_lora_models,
                            lambda: {"choices": sorted(get_lora_models())},
                            "refresh_lora_models",
                        )
                ...
                with gr.Tab("Settings", elem_id="TabSettings"):
                    ...
                    with gr.Accordion(open=True, label="Basic"):
                        ...
                        schedulers = [
                            ...
                            "cyclic_cosine_decay_with_warmup",
                        ]
                        with gr.Column():
                            ...
                            db_init_decay_epochs = gr.Number(
                                label="Init Decay Epochs",
                                value=10,
                                precision=0,
                                visible=False,
                            )
                            db_restart_interval = gr.Number(
                                label="Restart Interval",
                                value=10,
                                precision=0,
                                visible=False,
                            )
                            db_restart_interval_multiplier = gr.Number(
                                label="Restart Interval Multiplier",
                                value=1,
                                precision=0,
                                visible=False,
                            )
                            db_restart_lr = gr.Number(
                                label="Restart LR",
                                value=1e-6,
                                visible=False,
                            )
                            db_warmup_epochs = gr.Number(
                                label="Warmup Epochs",
                                value=10,
                                precision=0,
                                visible=False,
                            )
                            db_warmup_start_lr = gr.Number(
                                label="Warmup Start LR",
                                value=1e-6,
                                visible=False,
                            )
                            ...
                            with gr.Column():
                                with gr.Column():
                                    ...
                                    db_deis_train_scheduler = gr.Checkbox(
                                        label="Use DEIS for noise scheduler",
                                        value=False,
                                    )
                                    db_noise_train_scheduler = gr.Dropdown(
                                        label="Noise Train Scheduler",
                                        value="UniPC",
                                        choices=list_noise_train_schedulers(),
                                    )
        ...
        def toggle_lr_min(sched):
            ...
            show_decay_lr_min = gr.update(
                visible=sched == "cyclic_cosine_decay_with_restarts"
            )
            ...
            if sched == "cyclic_cosine_decay_with_restarts":
                show_init_decay_epochs = gr.update(visible=True)
                show_restart_interval = gr.update(visible=True)
                show_restart_lr = gr.update(visible=True)
                show_warmup_epochs = gr.update(visible=True)
                show_warmup_start_lr = gr.update(visible=True)
            return (
                show_lr_power,
                show_lr_cycles,
                show_scale_pos,
                ...
                show_lr_warmup,
                show_decay_lr_min,
                show_init_decay_epochs,
                show_restart_interval,
                show_restart_lr,
                show_warmup_epochs,
                show_warmup_start_lr,
            )
